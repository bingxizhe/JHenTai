...
class ArchiveDownloadService extends GetxController
  ...
  Future<void> ensureRestored() async {
    ...
    if (downloadSetting.restoreTasksAutomatically.isTrue) {
      log.info('ensureRestored: begin restoreTasks (scanning archive download directory)');
      await restoreTasks();
      log.info('ensureRestored: restoreTasks done');
  ...
  Future<void> downloadArchive(ArchiveDownloadedData archive,
    ...
    if (!resume) {
      ...
      ArchiveDownloadedData? archiveWithSanitizedTitle =
          await _initArchiveInfo(archive);
  ...
  Future<int> restoreTasks() async {
    ...
    await for (FileSystemEntity galleryDir in downloadDir.list()) {
      ...
      Map metadata = jsonDecode(await metadataFile.readAsString());
  ...
  Future<void> _generateComicInfoInDisk(ArchiveDownloadedData archive) async {
    ...
    try {
      File file =
          File(join(computeArchiveUnpackingPath(archive), 'ComicInfo.xml'));
  ...
  Future<ArchiveDownloadedData?> _initArchiveInfo(
      ArchiveDownloadedData archive) async {
    ...
    archive = archive.copyWith(
        sanitizedTitle:
            Value(_computeSanitizedArchiveTitle(archive.title, reservedBytes)));
  ...
  Future<void> _saveArchiveInfoInDisk(ArchiveDownloadedData archive) async {
    File file =
        File(join(computeArchiveUnpackingPath(archive), metadataFileName));
    if (!await file.exists()) {
      await file.create(recursive: true);
    }
